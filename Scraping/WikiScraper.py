...
class MariageScraper(Scraper):
    ...
    REGEX = r'(?P<date>[0-9]{4}.[0-9]{2}.[0-9]{2}|[0-9]{4}|[0-9]{4}.[0-9]{2}) \/ (?P<location>[a-zA-Z\-\èÉéÈ]+|-| |).' \
             r' Mariage (de |d\')(?P<name1>[a-zA-Z\-\èÉéÈ]+) (?P<lastName1>[a-zA-Z\-\èÉéÈ]+) (avec |et de |et d\'|et )' \
             r'(?P<name2>[a-zA-Z\-\èÉéÈ]+) (?P<lastName2>[a-zA-Z\-\èÉéÈ]+)'


    @staticmethod
    def extract(text):
        entities = set()
        result = re.finditer(MariageScraper.REGEX, text, re.UNICODE)
        for g in result:
            p1 = Person(g.group('name1'), g.group('lastName1'))
            p2 = Person(g.group('name2'), g.group('lastName2'))
            d = Date.extractDate(g.group('date'))
